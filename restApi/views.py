    ...
    if request.method == 'GET':
        ...
        try:
            ...
            for element in inventory_items:
                ...
                inventory_items_serializer = Inventory_ItemsSerializer(
                    data=dic_inventory)
            ...
            return HttpResponse(json.dumps({"error": "Inventory is Required"}), content_type="application/json", status=404)
...
def trade_items(request, pk, slug, month, username):
    ...
    if request.method == 'PUT':
        ...
        for i in range(0, len(types_items1)):
            if (i % 2 == 0):
                try:
                ...
                iv = Inventory_Items.objects.filter(
                    items=item.pk, survivor_id=survivor_1.id)
                if (iv.count() == 0):
        ...
        for i in range(0, len(types_items2)):
            if (i % 2 == 0):
                try:
                ...
                iv = Inventory_Items.objects.filter(
                    items=item.pk, survivor_id=survivor_2.id)
                if (iv.count() == 0):
                    return HttpResponse(json.dumps({"error": "Item in inventory does not exists"}), content_type="application/json", status=404)
        ...
        if (soma_total1 != soma_total2):
            ...
            for key in dic_items1:
                ...
                if inventory_items_serializer.is_valid():
                    inventory_items_serializer.save()
                Inventory_Items.objects.filter(
                    survivor_id=pk_1, items=items1.pk).first().delete()
            ...
            for key in dic_items2:
                ...
                if inventory_items_serializer.is_valid():
                    inventory_items_serializer.save()
                Inventory_Items.objects.filter(
                    survivor_id=pk_2, items=items2.pk).first().delete()
    ...
    if request.method == 'GET':
        ...
        for survivor in survivors:
            if survivor.is_infected:
                inventories = Inventory_Items.objects.filter(
